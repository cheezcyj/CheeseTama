...
현재 UI는 uGUI 기반이며, `StarterSceneBuilder`가 주요 씬 오브젝트와 UI를 프로그램 방식으로 재구성합니다. 디자인이 고정되기 전까지는 빌더 기반 생성을 유지하고, 런타임에서 반복 참조할 아트 에셋만 프리팹으로 고정합니다. UI 패널과 버튼의 둥근 배경은 Unity 버전에 따라 달라지는 내장 PSD 리소스에 의존하지 않고 런타임 생성 스프라이트를 사용합니다.
