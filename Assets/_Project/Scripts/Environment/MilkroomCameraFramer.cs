...
namespace CheeseTama.Environment
{
    ...
    public sealed class MilkroomCameraFramer : MonoBehaviour
    {
        ...
        private float currentSafeBottomFraction;

        public Vector2 NavigationOffset => navigationOffset;
        public float CurrentSafeBottomFraction => currentSafeBottomFraction;
        ...
        /// <summary>
        /// Applies the bounded offset produced by MilkroomViewportNavigationModel. A positive Y
        /// offset is rejected defensively because it could lower the character behind the
        /// persistent bottom bars.
        /// </summary>
        public void SetNavigationOffset(Vector2 offset)
        {
            navigationOffset = new Vector2(offset.x, Mathf.Min(0f, offset.y));
        }

        public void ResetNavigationOffset()
        {
            navigationOffset = Vector2.zero;
        }
        ...
        private void LateUpdate()
        {
            ...
            if (safeReferenceHeight <= 0f)
            {
                currentSafeBottomFraction = 0f;
                MoveCamera(
                    baselineX + navigationOffset.x,
                    Mathf.Clamp(
                        baselineY + navigationOffset.y,
                        baselineY - maxLift,
                        baselineY));
                return;
            }
            ...
            currentSafeBottomFraction = desiredBottomViewport;
            ...
            var framedY = Mathf.Clamp(requiredCamY, baselineY - maxLift, baselineY);
            var targetY = Mathf.Clamp(
                framedY + navigationOffset.y,
                baselineY - maxLift,
                baselineY);

            MoveCamera(baselineX + navigationOffset.x, targetY);
        }
        ...
        private void MoveCamera(float targetX, float targetY)
        {
