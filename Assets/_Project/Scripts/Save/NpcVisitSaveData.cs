...
namespace CheeseTama.Save
{
    ...
    public sealed class NpcVisitSaveData
    {
        ...
        public const int MaximumStainGuestWeeklyReceipts = 64;
        ...
        public string stainGuestEchoChoiceId = string.Empty;
        public bool stainGuestKeepsakeUnlocked;
        public List<string> stainGuestWeeklyReceiptKeys = new List<string>();
        ...
        public bool EnsureRuntimeDefaults()
        {
            ...
            changed |= EnsureString(ref stainGuestEchoChoiceId);
            ...
            stainGuestWeeklyReceiptKeys ??= new List<string>();
            changed |= NormalizeRelationships();
            changed |= NormalizeReceipts();
            changed |= NormalizeStableKeys(
                stainGuestWeeklyReceiptKeys,
                MaximumStainGuestWeeklyReceipts);
        ...
        private static bool NormalizeStableKeys(List<string> values, int maximumCount)
        {
            var changed = false;
            var known = new HashSet<string>(StringComparer.Ordinal);
            for (var index = values.Count - 1; index >= 0; index -= 1)
            {
                var normalized = (values[index] ?? string.Empty).Trim();
