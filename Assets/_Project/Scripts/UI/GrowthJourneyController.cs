...
namespace CheeseTama.UI
{
    public sealed class GrowthJourneyController : MonoBehaviour
    {
        ...
        public void Open()
        {
            var profile = UnityEngine.Object.FindFirstObjectByType<CheeseTamaProfileMenuController>();
            openedFromProfile = profile != null
                && profile.BeginNestedChildNavigation(ProfileChildPage.GrowthJourney);
            nestedProfileController = openedFromProfile ? profile : null;
            if (OpenInternal(false))
            {
                return;
            }

            ReturnToProfileIfNeeded();
        }
        ...
        private bool OpenInternal(bool unlockAnnouncement)
        {
        ...
        private void ReturnToProfileIfNeeded()
        {
            var profile = nestedProfileController;
            var shouldReturn = openedFromProfile;
            nestedProfileController = null;
            openedFromProfile = false;
            if (shouldReturn)
            {
                profile?.ReturnFromNestedChildNavigation(ProfileChildPage.GrowthJourney);
            }
        }
