    ...
    public sealed class NewGameSetupController : MonoBehaviour
    {
            ...
            DevPanelController developerPanelController = null)
        {
            ...
            if (selectionText != null)
            {
                ...
                selectionText.resizeTextMinSize = 13;
                selectionText.resizeTextMaxSize = Mathf.Max(16, selectionText.fontSize);

                var selectionSize = selectionText.rectTransform.sizeDelta;
                selectionSize.y = Mathf.Max(selectionSize.y, 64f);
                selectionText.rectTransform.sizeDelta = selectionSize;
            }
        ...
        private void RefreshFromState(NewGameSetupSaveData state)
        {
            ...
            SetText(titleText, isEggStep ? "마음에 드는 알을 골라 주세요" : "처음 먹일 우유를 골라 주세요");
            SetText(
                bodyText,
                isEggStep
                    ? "알마다 좋아하는 돌봄이 조금 달라요. 이름 아래 설명을 읽고 하나를 눌러 보세요."
                    : "우유마다 처음 자라는 모습이 조금 달라요. 설명을 읽고 하나를 눌러 보세요.");
        ...
        private static string BuildSelectionSummary(
            ...
            bool isEggStep)
        {
            var hasEgg = NewGameSetupCatalog.TryGetEgg(state.selectedEggId, out var egg);
            var eggName = hasEgg ? egg.DisplayName : "아직 선택하지 않음";
            var eggDescription = hasEgg
                ? GetPlayerFriendlyDescription(egg)
                : "알을 하나 눌러 주세요.";
            if (isEggStep)
            {
                return $"선택한 알: {eggName} — {eggDescription}";
            }

            var hasMilk = NewGameSetupCatalog.TryGetFirstMilk(
                state.selectedFirstMilkId,
                out var milk);
            var milkName = hasMilk ? milk.DisplayName : "아직 선택하지 않음";
            var milkDescription = hasMilk
                ? GetPlayerFriendlyDescription(milk)
                : "우유를 하나 눌러 주세요.";
            return $"선택한 알: {eggName} — {eggDescription}\n첫 우유: {milkName} — {milkDescription}";
        }
            ...
            bool stepActive)
        {
            for (var index = 0; index < buttons.Length; index++)
            {
                ...
                if (!hasChoice)
                {
                    AccessibilityRuntime.SetTextAndApply(labels[index], string.Empty);
                    continue;
                }
                ...
                AccessibilityRuntime.SetTextAndApply(
                    label,
                    $"<size=20>{(selected ? "✓ " : string.Empty)}<b>{choice.DisplayName}</b></size>\n"
                    + $"<size=13>{GetPlayerFriendlyDescription(choice)}</size>");
            }
        }

        internal static string GetPlayerFriendlyDescription(NewGameSetupChoiceDefinition choice)
        {
            if (choice == null)
            {
                return "마음에 드는 것을 골라 보세요.";
            }

            return choice.Id switch
            {
                NewGameSetupCatalog.CreamEggId => "처음 키우기 편하고 어떤 돌봄도 잘 받아요.",
                NewGameSetupCatalog.ButterEggId => "움직이고 노는 것을 좋아해요.",
                NewGameSetupCatalog.StrawberryEggId => "기분을 잘 보여 주고 다정해요.",
                NewGameSetupCatalog.MintEggId => "깨끗하고 조용한 방을 좋아해요.",
                NewGameSetupCatalog.CoffeeEggId => "천천히 생각하고 한 가지에 집중해요.",
                NewGameSetupCatalog.BasicFirstMilkId => "처음 먹이기 좋고 골고루 자라게 도와줘요.",
                NewGameSetupCatalog.WarmFirstMilkId => "마음을 편하게 하고 기분을 잘 보여 주게 도와줘요.",
                NewGameSetupCatalog.ColdFirstMilkId => "서두르지 않고 차분히 살펴보게 도와줘요.",
                NewGameSetupCatalog.NuttyFirstMilkId => "새 놀이에 씩씩하게 나서게 도와줘요.",
                NewGameSetupCatalog.CoffeeFirstMilkId => "한 가지 놀이에 오래 집중하게 도와줘요.",
                _ => string.IsNullOrWhiteSpace(choice.Description)
                    ? "마음에 드는 것을 골라 보세요."
                    : choice.Description
            };
        }
