    ...
    public sealed class DebugUIController : MonoBehaviour
    {
        ...
        public void Refresh()
        {
            ...
            foreach (var milk in MilkCatalog.MainMilks)
            {
                if (milk != null)
                {
                    builder.AppendLine(FormatMilkGrowthLine(milk.id, milk.displayName));
                }
            }
        ...
        private string FormatStarMilkLine()
        {
            ...
            return FormatMilkGrowthLine(MilkCatalog.StarMilkId, "별빛 우유");
        }
        ...
        private string FormatDailyRoutine()
        {
            ...
            if (daily == null)
            {
                return "오늘: 먹기 0/3 요리 0/2 놀이 0/3 청소 0/2 휴식 0/2";
            }

            var eatingCount = daily.milkFeeds + daily.snacksFed;
            return $"오늘: 먹기 {ClampGoal(eatingCount, DailyCareSaveData.EatingGoal)}/{DailyCareSaveData.EatingGoal} 요리 {ClampGoal(daily.cookings, DailyCareSaveData.CookingGoal)}/{DailyCareSaveData.CookingGoal} 놀이 {ClampGoal(daily.playSessions, DailyCareSaveData.PlayGoal)}/{DailyCareSaveData.PlayGoal} 청소 {ClampGoal(daily.cleanings, DailyCareSaveData.CleanGoal)}/{DailyCareSaveData.CleanGoal} 휴식 {ClampGoal(daily.rests, DailyCareSaveData.RestGoal)}/{DailyCareSaveData.RestGoal}, 완료 {daily.completedRoutineCount}";
        }
