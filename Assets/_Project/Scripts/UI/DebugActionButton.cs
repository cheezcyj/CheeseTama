...
namespace CheeseTama.UI
{
    ...
    public sealed class DebugActionButton : MonoBehaviour
    {
        ...
        [SerializeField] private MilkroomUIController milkroomUiController;
        ...
        private string ApplyAction(GameManager manager, CheeseTamaModel tama, ref bool celebrate, ref string eventId)
        {
            ...
            switch (action)
            {
                ...
                case DebugAction.AddLevelOne:
                    return AddLevels(tama, 1, ref celebrate);
                case DebugAction.AddLevelTwo:
                    return AddLevels(tama, 2, ref celebrate);
                case DebugAction.AddLevelFive:
                    return AddLevels(tama, 5, ref celebrate);
                default:
                    return "선택된 개발자 액션이 없습니다.";
            }
        }

        private static string AddLevels(CheeseTamaModel tama, int amount, ref bool celebrate)
        {
            var safeAmount = Mathf.Max(1, amount);
            var previousLevel = tama.level;
            if (previousLevel >= tama.maxLevel)
            {
                return $"이미 최대 레벨 {tama.maxLevel}입니다.";
            }

            var levelSystem = new LevelSystem();
            levelSystem.AddProgress(tama, safeAmount * 100);

            var hatched = new HatchingSystem().TryHatch(tama);
            celebrate = tama.level > previousLevel || hatched;
            var increased = tama.level - previousLevel;
            var hatchMessage = hatched ? " 알에서 부화했습니다." : string.Empty;
            return $"개발자 레벨 조정: +{increased}, 현재 Lv.{tama.level}.{hatchMessage}";
        }
        ...
        private void Refresh(string message, GameManager manager, bool celebrate, string eventId = "")
        {
            ...
            milkroomUiController?.Bind(manager.CurrentSave);
            milkroomUiController?.ShowMessage(message);
            ...
            if (visual != null)
            {
                ...
                {
                    if (celebrate && IsLevelAction(action))
                    {
                        visual.ReactAction(CheeseTamaVisualAction.LevelUp);
                    }
                    else
                    {
        ...
        private static bool IsLevelAction(DebugAction debugAction)
        {
            return debugAction == DebugAction.AddLevelOne
                || debugAction == DebugAction.AddLevelTwo
                || debugAction == DebugAction.AddLevelFive;
        }

        private void ResolveReferences()
        {
            if (uiController == null && milkroomUiController == null)
            {
                uiController = Object.FindFirstObjectByType<DebugUIController>();
                milkroomUiController = Object.FindFirstObjectByType<MilkroomUIController>();
            }
