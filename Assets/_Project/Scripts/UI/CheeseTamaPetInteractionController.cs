...
namespace CheeseTama.UI
{
    ...
    public sealed class CheeseTamaPetInteractionController : MonoBehaviour
    {
        ...
        private const float MaximumTapSeconds = 0.8f;
        ...
        /// <summary>
        /// Lets overlapping room props yield the pointer to the character. The pet
        /// collider is queried directly so a chair or shelf behind the model cannot
        /// turn a short character tap into its own action.
        /// </summary>
        public bool ContainsScreenPoint(Vector2 screenPosition)
        {
            ...
            return IsPointerOverCollider(screenPosition);
        }
        ...
        {
            FinishStrokeAt((Vector2)Input.mousePosition);
        }

        private void FinishStrokeAt(Vector2 releasePointerPosition)
        {
            if (!strokeActive)
            {
                return;
            ...
            var gameplayBlocked = IsGameplayBlocked();
            var releasedOverCollider = !gameplayBlocked
                && IsPointerOverCollider(releasePointerPosition);
            if (releasedOverCollider)
            {
                accumulatedStrokeDistance += Vector2.Distance(
                        previousPointerPosition,
                        releasePointerPosition)
                    / Mathf.Max(1f, Screen.height);
            }

            var releaseOutcome = ResolveReleaseOutcome(
                gameplayBlocked,
                releasedOverCollider,
                elapsed,
                accumulatedStrokeDistance);
            CancelStroke();
            switch (releaseOutcome)
            {
                case ReleaseOutcome.VisualTap:
                    ShowTapHeartReaction();
                    break;
                case ReleaseOutcome.PetStroke:
                    CommitPet();
                    break;
            }
        }

        private static ReleaseOutcome ResolveReleaseOutcome(
            bool gameplayBlocked,
            bool releasedOverCollider,
            float elapsed,
            float strokeDistance)
        {
            if (gameplayBlocked || !releasedOverCollider)
            {
                return ReleaseOutcome.None;
            }

            if (elapsed >= MinimumStrokeSeconds
                && strokeDistance >= MinimumStrokeScreenRatio)
            {
                return ReleaseOutcome.PetStroke;
            }

            return elapsed <= MaximumTapSeconds
                && strokeDistance < MinimumStrokeScreenRatio
                    ? ReleaseOutcome.VisualTap
                    : ReleaseOutcome.None;
        }

        private void ShowTapHeartReaction()
        {
            visualController?.ReactAction(CheeseTamaVisualAction.Pet);
        }
