...
namespace CheeseTama.UI
{
    ...
    public sealed class LifeRecordsPanelController : MonoBehaviour
    {
        ...
        [SerializeField] private ScrollRect overviewScrollRect;
        [SerializeField] private ScrollRect episodeScrollRect;
            ...
            Action<bool> onBlockingChanged = null)
        {
            ...
            overviewScrollRect = overviewText != null
                ? overviewText.GetComponentInParent<ScrollRect>(true)
                : null;
            episodeScrollRect = episodeText != null
                ? episodeText.GetComponentInParent<ScrollRect>(true)
                : null;
        ...
        public bool Open()
        {
            ...
            ResetScrollToTop(overviewScrollRect);
            ResetScrollToTop(episodeScrollRect);
            NotifyBlocking(true);
        ...
        private static string BuildOverviewText(LifeRecordsSnapshot value)
        {
            ...
            builder.Append("<b>미니게임 기록</b>\n");
            AppendMiniGameRecord(builder, value.MilkDrop);
            builder.Append('\n');
            AppendMiniGameRecord(builder, value.Cleaning);
            builder.Append('\n');
            AppendMiniGameRecord(builder, value.BouncyJump);
            builder.Append('\n');
            AppendMiniGameRecord(builder, value.BlueBall);

            builder.Append("\n\n<b>최근 미니게임 결과</b>\n");
            if (value.RecentMiniGameSessions.Count == 0)
            {
                builder.Append("아직 저장된 결과가 없어요.");
            }
            else
            {
                for (var index = 0; index < value.RecentMiniGameSessions.Count; index += 1)
                {
                    if (index > 0)
                    {
                        builder.Append('\n');
                    ...
                    var session = value.RecentMiniGameSessions[index];
                    builder.Append("• ");
                    builder.Append(EscapeRichText(FormatDate(session.CompletedAtIso)));
                    builder.Append(" · ");
                    builder.Append(EscapeRichText(session.DisplayName));
                    builder.Append(" · ");
                    builder.Append(session.Score);
                    builder.Append("점 · 성공 ");
                    builder.Append(session.Successes);
                    builder.Append("회");
                }
            }
        ...
        private static void AppendMiniGameRecord(
            StringBuilder builder,
            MiniGameLifeRecord record)
        {
            builder.Append("• ");
            builder.Append(EscapeRichText(record.DisplayName));
            builder.Append(" · ");
            if (!record.HasPlayed)
            {
            ...
            builder.Append("최고 ");
            builder.Append(record.HighestScore);
            builder.Append("점 · ");
            builder.Append(record.TotalSessions);
            builder.Append("회 플레이 · 성공 ");
            builder.Append(record.TotalSuccesses);
            builder.Append("회");
        }
