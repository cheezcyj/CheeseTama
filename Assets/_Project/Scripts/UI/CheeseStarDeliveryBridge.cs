...
namespace CheeseTama.UI
{
    public sealed class CheeseStarDeliveryBridge : MonoBehaviour
    {
        ...
        private void ApplyEntryPresentation(CheeseStarDeliveryOffer offer)
        {
            ...
            if (label != null)
            {
                label.text = ResolveEntryLabel(isClaimed, IsSquareEntryButton(entryButton));
            }
        ...
        public static string ResolveEntryLabel(bool claimed, bool square)
        {
            if (!square)
            {
                return claimed ? ClaimedEntryLabel : PendingEntryLabel;
            }

            return claimed ? "배달\n받음" : "오늘\n배달";
        }

        private static bool IsSquareEntryButton(Button button)
        {
            ...
            var width = Mathf.Abs(rect.sizeDelta.x) > 0.01f
                ? Mathf.Abs(rect.sizeDelta.x)
                : 0f;
            var height = Mathf.Abs(rect.sizeDelta.y) > 0.01f
                ? Mathf.Abs(rect.sizeDelta.y)
                : 0f;
            return width > 0f && height > 0f && width <= height * 1.55f;
        }
