...
namespace CheeseTama.UI
{
    public sealed class CollectionUIController : MonoBehaviour
    {
        ...
        private const string ClaimAllButtonName = "Claim All Collection Fragments Button";
        ...
        private readonly TextGenerator cardDetailTextGenerator = new TextGenerator();
        private CollectionRecordTab activeTab = CollectionRecordTab.Discovery;
        private bool tabsEnabled;
        private bool cardLayoutEnabled;
        private bool runtimeInitialized;
        private RectTransform discoveryCardRoot;
        ...
        private Button claimAllFragmentsButton;
        private CheeseTamaSaveData boundSaveData;
        private bool fragmentRewardCapacityAvailable;
        private float cardTextScale = GameSettingsSaveData.DefaultTextScale;
        ...
        private bool HasCardRoots()
        {
            ...
            return content != null
                && content.Find("Discovery Records Card Root") != null
            ...
            Button hiddenTab)
        {
            ...
            ConfigureDiscoveryTabPresentation();
        ...
        public void Bind(CheeseTamaSaveData saveData)
        {
            ...
            RefreshClaimAllFragmentButton(saveData);
            RefreshRewardNotificationBadges(saveData.collections, fragmentRewardCapacityAvailable);
            var discoveryRecords = new List<string>(
                saveData.collections.milk.Count + saveData.collections.events.Count);
            discoveryRecords.AddRange(saveData.collections.milk);
            discoveryRecords.AddRange(saveData.collections.events);
            SetText(milkText, FormatRecordList("발견 기록", discoveryRecords, FormatKnownRecordName, careerBenefits));
        ...
        {
            cardTextScale = GameSettingsSaveData.NormalizeTextScale(
                saveData?.settings?.textScale ?? GameSettingsSaveData.DefaultTextScale);
            EnsureCardRoots();
            ...
            {
                discoveryCardCount = 0;
                evolutionCardCount = 0;
                hiddenCardCount = 0;
                BuildCollectionCards(discoveryCardRoot, "발견 기록", new List<CollectionCardData>(), "아직 발견한 우유나 이벤트 기록이 없습니다.");
            ...
            var hiddenCards = CreateHiddenRecordCards(
                ...
                careerBenefits);
            var discoveryCards = new List<CollectionCardData>(
                milkCards.Count + eventCards.Count);
            discoveryCards.AddRange(milkCards);
            discoveryCards.AddRange(eventCards);
            discoveryCardCount = discoveryCards.Count;
        ...
        private void EnsureCardRoots()
        {
            ...
            RemoveLegacyCardRoot(content, "Milk Records Card Root");
            RemoveLegacyCardRoot(content, "Event Records Card Root");
            discoveryCardRoot = GetOrCreateCardRoot(content, "Discovery Records Card Root");
        ...
        private static void RemoveLegacyCardRoot(RectTransform content, string rootName)
        {
            var legacy = content != null ? content.Find(rootName) : null;
            if (legacy == null)
            {
                return;
            }
            ...
            HiddenCareerBenefitSet careerBenefits = default)
        {
            ...
            for (var i = 0; i < records.Count; i++)
            {
                ...
                {
                    EnsureGrowthVisualSet();
                    thumbnail = CheeseTamaProfilePortraitCatalog.LoadPortrait(growthStage.Stage);
                    if (thumbnail == null && growthVisualSet != null)
                    {
                        thumbnail = growthVisualSet.GetThumbnail(growthStage.Stage);
                    }
                }
        ...
        private float CalculateCardHeight(IList<CollectionCardData> cards)
        {
            var requiredHeight = CardHeight;
            if (cards != null)
            {
                for (var index = 0; index < cards.Count; index += 1)
                {
                    ...
                    var detailWidth = card.HasFragmentReward
                        ? CardWidth - 302f
                        : CardWidth - 100f;
                    var preferredHeight = CalculatePreferredDetailHeight(detail, detailWidth);
                    requiredHeight = Mathf.Max(
                        requiredHeight,
                        62f + Mathf.Ceil(preferredHeight));
                }
            }
        ...
        private float CalculatePreferredDetailHeight(string detail, float width)
        {
            var template = milkText ?? recordHeaderText ?? evolutionText ?? eventText ?? hiddenText;
            var fontSize = Mathf.Max(1, Mathf.RoundToInt(14f * cardTextScale));
            if (template == null || template.font == null)
            {
                var charactersPerLine = Mathf.Max(1, Mathf.FloorToInt(width / (fontSize * 0.85f)));
                var estimatedLines = Mathf.Max(
                    1,
                    Mathf.CeilToInt((detail?.Length ?? 0) / (float)charactersPerLine));
                return estimatedLines * fontSize * 1.2f;
            }

            var settings = template.GetGenerationSettings(new Vector2(Mathf.Max(1f, width), 10000f));
            settings.fontSize = fontSize;
            settings.fontStyle = FontStyle.Normal;
            settings.resizeTextForBestFit = false;
            settings.horizontalOverflow = HorizontalWrapMode.Wrap;
            settings.verticalOverflow = VerticalWrapMode.Overflow;
            settings.lineSpacing = 1.08f;
            settings.generationExtents = new Vector2(Mathf.Max(1f, width), 10000f);
            return cardDetailTextGenerator.GetPreferredHeight(detail ?? string.Empty, settings);
        }
        ...
        private void EnsureClaimAllFragmentButton()
        {
            ...
            var existing = parent.Find(ClaimAllButtonName);
            if (existing == null)
            {
                existing = parent.Find(MilkClaimAllButtonName)
                    ?? parent.Find(EvolutionClaimAllButtonName)
                    ?? parent.Find(EventClaimAllButtonName)
                    ?? parent.Find(HiddenClaimAllButtonName);
                if (existing != null)
                {
                    existing.name = ClaimAllButtonName;
                }
            }

            claimAllFragmentsButton = EnsureClaimAllFragmentButton(parent, existing);
            RemoveLegacyClaimAllButtons(parent);
        }

        private Button EnsureClaimAllFragmentButton(Transform parent, Transform existing)
        {
            ...
            {
                buttonObject = new GameObject(ClaimAllButtonName);
                buttonObject.transform.SetParent(parent, false);
            }
            ...
            button.onClick.AddListener(ClaimActiveTabCollectionFragmentRewards);
        ...
        private void RemoveLegacyClaimAllButtons(Transform parent)
        {
            RemoveLegacyClaimAllButton(parent, MilkClaimAllButtonName);
            RemoveLegacyClaimAllButton(parent, EvolutionClaimAllButtonName);
            RemoveLegacyClaimAllButton(parent, EventClaimAllButtonName);
            RemoveLegacyClaimAllButton(parent, HiddenClaimAllButtonName);
        }

        private static void RemoveLegacyClaimAllButton(Transform parent, string buttonName)
        {
            var legacy = parent != null ? parent.Find(buttonName) : null;
            if (legacy == null)
            {
                return;
            }
        ...
        private void ClaimActiveTabCollectionFragmentRewards()
        {
            var manager = GameManager.Instance;
            if (manager == null)
            {
                if (claimAllFragmentsButton != null)
                {
                    claimAllFragmentsButton.interactable = true;
                }
            ...
            if (claimAllFragmentsButton != null)
            {
                claimAllFragmentsButton.interactable = false;
            }

            if (activeTab == CollectionRecordTab.Discovery)
            {
                manager.ClaimDiscoveryCollectionFragmentRewards();
            }
            else if (activeTab == CollectionRecordTab.Evolution)
            {
                manager.ClaimCollectionFragmentRewards(CollectionRecordCategory.Evolution);
            }
            else
            {
                manager.ClaimCollectionFragmentRewards(CollectionRecordCategory.Hidden);
            }

            Bind(manager.CurrentSave);
        }

        private void RefreshClaimAllFragmentButton(CheeseTamaSaveData saveData)
        {
            EnsureClaimAllFragmentButton();
            if (claimAllFragmentsButton == null)
            {
            ...
            var labelTransform = claimAllFragmentsButton.transform.Find("Label");
            ...
            var unclaimedCount = CountActiveTabUnclaimedFragmentRewards(collections);
            ...
            var claimableCount = remainingCapacity > 0L
                ? System.Math.Min((long)unclaimedCount, remainingCapacity)
                : 0L;
            claimAllFragmentsButton.interactable = claimableCount > 0L;
            claimAllFragmentsButton.gameObject.SetActive(true);
            if (label == null)
            {
                return;
            }

            if (claimableCount > 0L)
            {
                label.text = $"도감조각 받기 ({claimableCount})";
            }
            ...
            {
                label.text = HasActiveTabRecord(collections)
                    ? "도감조각 받기 완료"
                    : "받을 도감조각 없음";
            }

            label.color = claimableCount > 0L
                ? new Color(0.28f, 0.16f, 0.05f)
                : new Color(0.38f, 0.34f, 0.28f);
            AccessibilityRuntime.ApplyCurrent(label);
        }

        private int CountActiveTabUnclaimedFragmentRewards(CollectionSaveData collections)
        {
            if (activeTab == CollectionRecordTab.Evolution)
            {
                return collectionSystem.CountUnclaimedFragmentRewards(
                    collections,
                    CollectionRecordCategory.Evolution);
            }

            if (activeTab == CollectionRecordTab.Hidden)
            {
                return collectionSystem.CountUnclaimedFragmentRewards(
                    collections,
                    CollectionRecordCategory.Hidden);
            ...
            var discoveryCount = (long)collectionSystem.CountUnclaimedFragmentRewards(
                    collections,
                    CollectionRecordCategory.Milk)
                + collectionSystem.CountUnclaimedFragmentRewards(
                    collections,
                    CollectionRecordCategory.Event);
            return (int)System.Math.Min(discoveryCount, int.MaxValue);
        }

        private bool HasActiveTabRecord(CollectionSaveData collections)
        {
            if (collections == null)
            {
                return false;
            ...
            if (activeTab == CollectionRecordTab.Hidden)
            {
                return collections.hiddenUnlockedOnly.Count > 0;
            }

            return collections.milk.Count > 0 || collections.events.Count > 0;
        }
            ...
            bool hasFragmentCapacity = false)
        {
            SetRewardNotificationBadge(
                ...
                hasFragmentCapacity
                    && (collectionSystem.CountUnclaimedFragmentRewards(
                            collections,
                            CollectionRecordCategory.Milk) > 0
                        || collectionSystem.CountUnclaimedFragmentRewards(
                            collections,
                            CollectionRecordCategory.Event) > 0));
            SetRewardNotificationBadge(
                evolutionTabButton,
                hasFragmentCapacity
        ...
        internal static string FormatKnownRecordName(string id)
        {
            if (string.Equals(
                    id,
            ...
            {
                return "얼룩 손님의 푸른 실 기념품";
            }

            if (string.Equals(id, "npc_visit_stain_guest", StringComparison.Ordinal))
            {
                return "얼룩 손님의 반복 방문";
            }

            if (DreamStorySeasonContentLoader.TryResolveCodexRecord(id, out var dreamCodex))
            {
                return dreamCodex.Title;
            }

            if (MilkroomMysteryContentLoader.TryResolveCodexRecord(id, out var mysteryCodex))
            {
                return mysteryCodex.Title;
            }

            if (LifeChapterSystem.TryResolveCodexRecord(id, out var lifeChapterCodex))
            {
                return lifeChapterCodex.Title;
            }

            if (MilkroomInvestigationContentLoader.TryResolveCodexRecord(
                    id,
                    out var investigationCodex))
            {
                return investigationCodex.Title;
            }

            var postgameResearch = PostgameResearchCatalog.Find(id);
            if (postgameResearch != null)
            {
                return postgameResearch.Title;
            }
            ...
            foreach (var milk in MilkCatalog.VisibleMilks)
            {
                ...
                if (TryParseRecordLevel(
                        id,
                        growthPrefix,
                        1,
                        UnlockSystem.MaxLevel,
                        out var growthLevel))
                {
                    return $"{milk.displayName} 레벨 {growthLevel} 달성";
                }
            }
            ...
            foreach (var milk in MilkCatalog.VisibleMilks)
            {
                ...
                if (TryParseRecordLevel(
                        id,
                        rewardPrefix,
                        MilkGrowthMilestoneRewardSystem.FirstRewardLevel,
                        MilkGrowthMilestoneRewardSystem.MaximumRewardLevel,
                        out var rewardLevel))
                {
                    return $"{milk.displayName} 특별 성장 레벨 {rewardLevel}";
                }
            }
            ...
            if (TryParseRecordLevel(
                    id,
                    BasicMilkGrowthPrefix,
                    1,
                    UnlockSystem.MaxLevel,
                    out var basicGrowthLevel))
            {
                return $"기본 우유 레벨 {basicGrowthLevel} 달성";
            }

            const string StarMilkGrowthPrefix = "star_milk_growth_lv_";
            if (TryParseRecordLevel(
                    id,
                    StarMilkGrowthPrefix,
                    1,
                    UnlockSystem.MaxLevel,
                    out var starGrowthLevel))
            {
                return $"별빛 우유 레벨 {starGrowthLevel} 달성";
            }
        ...
        private static bool TryParseRecordLevel(
            string recordId,
            string prefix,
            int minimum,
            int maximum,
            out int level)
        {
            ...
            var suffix = recordId.Substring(prefix.Length);
            return int.TryParse(
                    suffix,
                    NumberStyles.None,
        ...
        {
            if (DreamStorySeasonContentLoader.TryResolveCodexRecord(id, out _))
            {
                return "꿈 이야기";
            }

            if (MilkroomMysteryContentLoader.TryResolveCodexRecord(id, out _))
            {
                return "미스터리";
            }

            if (LifeChapterSystem.TryResolveCodexRecord(id, out _))
            {
                return "생활 챕터";
            }

            if (MilkroomInvestigationContentLoader.TryResolveCodexRecord(id, out _))
            {
                return "방 조사";
            }

            if (PostgameResearchCatalog.Find(id) != null)
            {
                return "성장 연구";
            }
        ...
        private static string FormatKnownRecordDetail(string id)
        {
            if (string.Equals(
                    ...
                    StringComparison.Ordinal))
            {
                return "꿈 이야기 시즌 1의 마지막 선택을 기억하며, 얼룩 손님이 첫 재방문에 남긴 푸른 실입니다.";
            }

            if (string.Equals(id, "npc_visit_stain_guest", StringComparison.Ordinal))
            {
                return "얼룩 손님이 다시 찾아와 이전 선택에 맞는 인사를 건네고, 주간 기록을 이어 간 방문입니다.";
            }

            if (DreamStorySeasonContentLoader.TryResolveCodexRecord(id, out var dreamCodex))
            {
                return dreamCodex.Detail;
            }

            if (MilkroomMysteryContentLoader.TryResolveCodexRecord(id, out var mysteryCodex))
            {
                return mysteryCodex.Detail;
            }

            if (LifeChapterSystem.TryResolveCodexRecord(id, out var lifeChapterCodex))
            {
                return lifeChapterCodex.Detail;
            }
            ...
            if (MilkroomInvestigationContentLoader.TryResolveCodexRecord(
                    ...
                    out var investigationCodex))
            {
                return investigationCodex.Detail;
            }

            var postgameResearch = PostgameResearchCatalog.Find(id);
            if (postgameResearch != null)
            {
                return PostgameResearchPanelController.SimplifyResearchCopy(
                    postgameResearch.Detail);
            }
        ...
        {
            ConfigureDiscoveryTabPresentation();
            ConfigureTabButton(milkTabButton, CollectionRecordTab.Discovery);
        ...
        private void ConfigureDiscoveryTabPresentation()
        {
            if (milkTabButton != null)
            {
                var label = milkTabButton.GetComponentInChildren<Text>(true);
                if (label != null)
                {
                    label.text = "발견";
                }

                milkTabButton.gameObject.SetActive(true);
            }

            if (eventTabButton != null)
            {
                eventTabButton.onClick.RemoveAllListeners();
                eventTabButton.gameObject.SetActive(false);
            }
        }
        ...
        {
            activeTab = tab;
            RefreshClaimAllFragmentButton(boundSaveData);
            if (cardLayoutEnabled)
            {
                ...
                SetCardRootVisible(discoveryCardRoot, tab == CollectionRecordTab.Discovery);
            ...
            SetTextVisible(milkText, tab == CollectionRecordTab.Discovery);
        ...
        {
            UpdateTabVisual(milkTabButton, activeTab == CollectionRecordTab.Discovery);
