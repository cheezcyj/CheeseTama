...
namespace CheeseTama.UI
{
    ...
    public sealed class CheeseTamaProfileMenuController : MonoBehaviour
    {
        ...
        private ProfileChildPage activeChildPage;
            ...
            Button replayTutorialButton = null)
        {
            ...
            tutorialReplayButton = replayTutorialButton;
            topMenuController = menuController;
            bottomActionBarController = actionBarController;
            devPanelController = developerPanelController;
        ...
        /// <summary>
        /// Starts a profile-owned detail page without ending the profile navigation session.
        /// The profile card is hidden while the child is visible, but its suspended controls
        /// and the launcher focus captured by <see cref="Open"/> are deliberately preserved.
        /// </summary>
        public bool BeginNestedChildNavigation(ProfileChildPage page)
        {
            if (page == ProfileChildPage.None
                || !IsOpen
                || activeChildPage != ProfileChildPage.None)
            {
                return false;
            }
        ...
        /// <summary>
        /// Restores the still-open profile session and focuses the entry that launched the child.
        /// Calls without a matching active child are ignored so initialization-time Close calls
        /// cannot open the profile unexpectedly.
        /// </summary>
        public void ReturnFromNestedChildNavigation(ProfileChildPage page)
        {
            if (page == ProfileChildPage.None || activeChildPage != page)
            {
                return;
            }
            ...
            overlayRoot.SetActive(true);
            overlayRoot.transform.SetAsLastSibling();
            EventSystem.current?.SetSelectedGameObject(ResolveChildEntry(page));
        }
        ...
        public void RefreshProfile()
        {
            ...
            if (profileDetailText != null)
            {
                ...
                profileDetailText.text = tama != null
                    ? $"레벨 {Mathf.Max(1, tama.level)} · {stageName}"
                    : stageName;
            }
            ...
            var portrait = CheeseTamaProfilePortraitCatalog.LoadPortrait(stage);
            if (portrait == null)
            {
                growthVisualSet ??= Resources.Load<CheeseTamaGrowthVisualSet>("CheeseTamaGrowthVisualSet");
                portrait = growthVisualSet != null
                    ? growthVisualSet.GetThumbnail(stage)
                    : null;
            }
        ...
        private GameObject ResolveChildEntry(ProfileChildPage page)
        {
            var entry = page switch
            {
                ProfileChildPage.GrowthJourney => growthJourneyButton,
                ProfileChildPage.MemoryJournal => memoryJournalButton,
                ProfileChildPage.BondStatus => bondStatusButton,
                _ => null
            };
