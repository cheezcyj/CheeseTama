...
namespace CheeseTama.UI
{
    public sealed class InputBindingsPanelController : MonoBehaviour
    {
        ...
        private bool restoreControlsInLateUpdate;
        ...
        public void Open()
        {
            ...
            if (restoreControlsInLateUpdate)
            {
                RestoreControls();
            }
        ...
        public void Cancel()
        {
            // Keep TopMenu disabled through the rest of this Update frame so the
            // same cancel key cannot cascade from this child overlay into the
            // still-open Settings modal. EditMode calls have no frame loop.
            CloseOverlay(Application.isPlaying);
        }

        private void CloseOverlay(bool deferControlRestore)
        {
            if (!IsOpen)
            {
                return;
            ...
            listeningActionId = string.Empty;
            SetVisible(false);
            if (deferControlRestore)
            {
                restoreControlsInLateUpdate = true;
            }
            else
            {
                RestoreControls();
        ...
        private void WireButtons()
        {
            ...
            if (closeButton != null)
            {
                ...
                closeButton.onClick.AddListener(Confirm);
            }
        }
        ...
        private void PrepareForReconfigure()
        {
            listeningActionId = string.Empty;
            restoreControlsInLateUpdate = false;
            SetVisible(false);
            RestoreControls();
            previousSelectedObject = null;
        }
