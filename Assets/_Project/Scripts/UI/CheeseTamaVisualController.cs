...
namespace CheeseTama.UI
{
    ...
    public sealed class CheeseTamaVisualController : MonoBehaviour
    {
        ...
        private const int PetHeartTextureSize = 128;
        private const int PetHeartSupersampling = 4;
        private const float PetHeartPixelsPerUnit = 128f;
        private const float PetHeartLocalScale = 0.3f;
        private const float PetHeartCoordinateSpan = 3f;
        ...
        private float modelBottomLocalY = -0.9f;
        private bool hasModelBottomLocalY;
        ...
        private static Texture2D cachedPetHeartTexture;
        private static Sprite cachedPetHeartSprite;
        ...
        private void CaptureModelBottomLocalY()
        {
            if (hasModelBottomLocalY || modelRenderers == null)
            {
                return;
            }
                    ...
                    for (var y = -1; y <= 1; y += 2)
                    {
                        for (var z = -1; z <= 1; z += 2)
                        {
                            var corner = bounds.center + Vector3.Scale(
                                bounds.extents,
                                new Vector3(x, y, z));
                            var localY = transform.InverseTransformPoint(corner).y;
                            if (float.IsNaN(localY) || float.IsInfinity(localY))
                            {
                                continue;
                            }

                            if (!foundPoint || localY < lowestY)
                            {
                                lowestY = localY;
                                foundPoint = true;
                            }
            ...
            if (foundPoint)
            {
                modelBottomLocalY = lowestY;
                hasModelBottomLocalY = true;
            }
        }
        ...
        private void LateUpdate()
        {
            ...
            if (activeAction == CheeseTamaVisualAction.Pet)
            {
                var yFactor = 1f + punch;
                var xzFactor = 1f / Mathf.Sqrt(Mathf.Max(0.0001f, yFactor));
                var pivotCompensationY = -modelBottomLocalY * baseScale.y * punch;
                transform.localPosition = restingLocalPosition
                    + Vector3.up * (hop + pivotCompensationY)
                    + Vector3.right * side;
        ...
        public void ReactAction(CheeseTamaVisualAction action, bool celebrate = false)
        {
            ...
            if (activeAction == CheeseTamaVisualAction.Pet)
            {
                PreparePetHeartSprite(petHeartRoot);
            }
        ...
        private void EnsureConditionIndicator()
        {
            ...
            if (messyConditionRoot == null)
            {
                ...
                var dustLarge = EnsurePrimitive(
                    ...
                    "Dust Large",
                    PrimitiveType.Sphere,
                    new Color(0.61f, 0.48f, 0.32f),
                    ToonMaterialProfile.CharacterMark);
                var dustMid = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Mid",
                    PrimitiveType.Sphere,
                    new Color(0.72f, 0.59f, 0.41f),
                    ToonMaterialProfile.CharacterMark);
                var dustSmall = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Small",
                    PrimitiveType.Sphere,
                    new Color(0.53f, 0.41f, 0.29f),
                    ToonMaterialProfile.CharacterMark);
                var dustPuff = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Puff",
                    PrimitiveType.Sphere,
                    new Color(0.82f, 0.70f, 0.51f),
                    ToonMaterialProfile.CharacterMark);
                var dustMoteLeft = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Mote Left",
                    PrimitiveType.Sphere,
                    new Color(0.48f, 0.36f, 0.24f),
                    ToonMaterialProfile.CharacterMark);
                var dustMoteRight = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Mote Right",
                    PrimitiveType.Sphere,
                    new Color(0.67f, 0.52f, 0.34f),
                    ToonMaterialProfile.CharacterMark);
                var dustHighlight = EnsurePrimitive(
                    messyConditionRoot,
                    "Dust Highlight",
                    PrimitiveType.Sphere,
                    new Color(1f, 0.9f, 0.68f),
                    ToonMaterialProfile.CharacterHighlight);
                SetPart(dustLarge, new Vector3(-0.08f, -0.035f, -0.045f), new Vector3(0.072f, 0.062f, 0.018f), Quaternion.identity);
                SetPart(dustMid, new Vector3(0.015f, -0.005f, -0.046f), new Vector3(0.068f, 0.07f, 0.017f), Quaternion.identity);
                SetPart(dustSmall, new Vector3(0.095f, -0.04f, -0.045f), new Vector3(0.05f, 0.048f, 0.014f), Quaternion.identity);
                SetPart(dustPuff, new Vector3(-0.015f, 0.04f, -0.047f), new Vector3(0.047f, 0.044f, 0.014f), Quaternion.identity);
                SetPart(dustMoteLeft, new Vector3(-0.12f, 0.065f, -0.047f), new Vector3(0.018f, 0.018f, 0.01f), Quaternion.identity);
                SetPart(dustMoteRight, new Vector3(0.115f, 0.075f, -0.047f), new Vector3(0.014f, 0.014f, 0.009f), Quaternion.identity);
                SetPart(dustHighlight, new Vector3(-0.036f, 0.052f, -0.052f), new Vector3(0.012f, 0.012f, 0.007f), Quaternion.identity);
            }
        ...
        private void RefreshConditionIndicator(bool force)
        {
            ...
            var hideSleepyForPet = condition == CareCondition.Sleepy
                && isReacting
                && activeAction == CheeseTamaVisualAction.Pet;
            ...
            if (!force
                ...
                && conditionRoot != null
                && conditionRoot.gameObject.activeSelf == visible)
            {
                return;
            }
        ...
        private void EnsureActionProps()
        {
            ...
            EnsurePetHeartSprite(petHeartRoot);
        ...
        private void UpdateActionProps(float normalized, float arc, float settle)
        {
            ...
            if (AccessibilityRuntime.ReducedMotion)
            {
                if (activeAction == CheeseTamaVisualAction.Pet)
                {
                    ...
                    SetPart(
                        petHeartRoot,
                        ResolvePetHeartPosition(0.18f),
                        ResolvePetHeartScale(1f),
                        Quaternion.identity);
                }
            ...
            switch (activeAction)
            {
                ...
                case CheeseTamaVisualAction.Cook:
                    ...
                    break;
                case CheeseTamaVisualAction.Pet:
                    var petHeartPosition = ResolvePetHeartPosition(0.18f + normalized * 0.18f);
                    petHeartPosition.x += Mathf.Sin(normalized * Mathf.PI * 2f) * 0.035f;
                    SetActive(petHeartRoot, true);
                    SetPart(
                        petHeartRoot,
                        petHeartPosition,
                        ResolvePetHeartScale(0.85f + arc * 0.28f),
                        Quaternion.identity);
                    break;
                case CheeseTamaVisualAction.MilkBlend:
        ...
        private static Transform EnsurePetHeartSprite(Transform parent)
        {
            RemoveGeneratedChild(parent, "Pet Heart Text");
            RemoveGeneratedChild(parent, "Pet Heart Left");
            RemoveGeneratedChild(parent, "Pet Heart Right");
            RemoveGeneratedChild(parent, "Pet Heart Base");

            var heart = EnsureChild(parent, "Pet Heart Sprite");
            var renderer = heart.GetComponent<SpriteRenderer>();
            if (renderer == null)
            {
                renderer = heart.gameObject.AddComponent<SpriteRenderer>();
            }

            renderer.sprite = GetOrCreatePetHeartSprite();
            renderer.color = new Color(1f, 0.24f, 0.43f, 1f);
            ...
            SetPart(
                heart,
                Vector3.zero,
                Vector3.one * PetHeartLocalScale,
                Quaternion.identity);
            return heart;
        }

        private static void PreparePetHeartSprite(Transform parent)
        {
            var renderer = parent?.Find("Pet Heart Sprite")?.GetComponent<SpriteRenderer>();
        ...
        private static Sprite GetOrCreatePetHeartSprite()
        {
            if (cachedPetHeartSprite != null && cachedPetHeartTexture != null)
            {
                return cachedPetHeartSprite;
            }

            cachedPetHeartTexture = new Texture2D(
                PetHeartTextureSize,
                PetHeartTextureSize,
                TextureFormat.RGBA32,
                false)
            {
                name = "Pet Heart 2D Texture",
                filterMode = FilterMode.Bilinear,
                wrapMode = TextureWrapMode.Clamp,
                hideFlags = HideFlags.HideAndDontSave
            };

            var pixels = new Color32[PetHeartTextureSize * PetHeartTextureSize];
            var samplesPerPixel = PetHeartSupersampling * PetHeartSupersampling;
            for (var pixelY = 0; pixelY < PetHeartTextureSize; pixelY += 1)
            {
                for (var pixelX = 0; pixelX < PetHeartTextureSize; pixelX += 1)
                {
                    var insideSamples = 0;
                    for (var sampleY = 0; sampleY < PetHeartSupersampling; sampleY += 1)
                    {
                        for (var sampleX = 0; sampleX < PetHeartSupersampling; sampleX += 1)
                        {
                            var normalizedX = (
                                pixelX + (sampleX + 0.5f) / PetHeartSupersampling)
                                / PetHeartTextureSize;
                            var normalizedY = (
                                pixelY + (sampleY + 0.5f) / PetHeartSupersampling)
                                / PetHeartTextureSize;
                            var heartX = (normalizedX - 0.5f) * PetHeartCoordinateSpan;
                            var heartY = (normalizedY - 0.5f) * PetHeartCoordinateSpan;
                            var squaredRadius = heartX * heartX + heartY * heartY;
                            var radiusOffset = squaredRadius - 1f;
                            var heartEquation = radiusOffset * radiusOffset * radiusOffset
                    ...
                    var alpha = (byte)Mathf.RoundToInt(
                        255f * insideSamples / samplesPerPixel);
                    pixels[pixelY * PetHeartTextureSize + pixelX] = new Color32(
                        255,
                        255,
                        255,
                        alpha);
            ...
            cachedPetHeartTexture.SetPixels32(pixels);
            cachedPetHeartTexture.Apply(false, false);
            cachedPetHeartSprite = Sprite.Create(
                cachedPetHeartTexture,
                new Rect(0f, 0f, PetHeartTextureSize, PetHeartTextureSize),
                new Vector2(0.5f, 0.5f),
                PetHeartPixelsPerUnit,
                0,
                SpriteMeshType.FullRect);
            cachedPetHeartSprite.name = "Pet Heart Borderless 2D Sprite";
            cachedPetHeartSprite.hideFlags = HideFlags.HideAndDontSave;
            return cachedPetHeartSprite;
        }

        private Vector3 ResolvePetHeartPosition(float verticalOffset)
        {
            var hasBounds = false;
            var bounds = new Bounds();
            if (modelRenderers != null)
            ...
            if (!hasBounds)
            {
                return new Vector3(0.36f, 0.36f + verticalOffset, PropZ);
            }

            var upperRightWorld = new Vector3(
                bounds.center.x + bounds.extents.x * 0.45f,
                bounds.max.y,
                bounds.center.z);
            var upperRightLocal = transform.InverseTransformPoint(upperRightWorld);
            return new Vector3(upperRightLocal.x, upperRightLocal.y + verticalOffset, PropZ);
        }

        private Vector3 ResolvePetHeartScale(float scale)
        {
            var parentScale = transform.lossyScale;
            var worldScaleX = Mathf.Abs(parentScale.x);
            var worldScaleY = Mathf.Abs(parentScale.y);
            var compensateX = worldScaleX > 0.0001f
                ? worldScaleY / worldScaleX
                : 1f;
            return new Vector3(scale * compensateX, scale, scale);
        }
        ...
        private static void DisableOverlayShadows(Transform part)
        {
            if (part == null || !part.TryGetComponent(out Renderer renderer))
            {
                return;
