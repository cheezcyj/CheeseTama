    ...
    public sealed class MilkroomCareButton : MonoBehaviour
    {
        ...
        private void HandleClick()
        {
            ...
            if (action == MilkroomCareAction.CatchMilkDrops)
            {
                ...
                {
                    Refresh(
                        $"{Mathf.CeilToInt(MilkDropMiniGameRules.DurationSeconds)}초 동안 그릇을 좌우로 움직여 우유방울을 받아 주세요.",
                        manager,
                        false);
                }
                else
            ...
            if (action == MilkroomCareAction.Clean)
            {
                ...
                {
                    Refresh(
                        $"{Mathf.CeilToInt(CleaningMiniGameRules.DurationSeconds)}초 동안 고정된 얼룩을 좌우로 문질러 닦아 주세요.",
                        manager,
                        false);
                }
                else
            ...
            if (action == MilkroomCareAction.Blend)
            {
                ...
                {
                    Refresh("요리와 우유 섞기 중 하나를 골라 주세요.", manager, false);
                }
                else
                {
            ...
            var displayMessage = FormatCareResultMessage(
                action,
                careResult,
                careActions.LastFeedingStatusResult,
                routineMessage,
                discoveryMessage);
        ...
        private CareActionResult RunCareAction(GameManager manager, MilkDefinition milkDefinition)
        {
            ...
            careActions.ConfigureRecoveryEffectPercent(
                manager.GetCareRecoveryEffectPercent());
        ...
        private static string RegisterMilkProgress(GameManager manager, MilkDefinition milk)
        {
            ...
            {
                message = $"{milk.displayName} 성장 레벨 {growth.growthLevel}이 되었어요!";
            }
            ...
            if (!starWasUnlocked && starNowUnlocked)
            {
                ...
                message = CombineMessages(message, "별빛 알과 별빛 우유가 새로 열렸어요!");
            }
        ...
        private static string RegisterNewMainMilkUnlocks(GameManager manager, Dictionary<string, bool> unlockedBefore)
        {
            ...
            foreach (var milk in MilkCatalog.MainMilks)
            {
                ...
                message = CombineMessages(message, $"{milk.displayName}가 새로 열렸어요!");
            }
        ...
        internal static string FormatCareResultMessage(
            MilkroomCareAction careAction,
            CareActionResult careResult,
            CheeseTama.Gameplay.Feeding.FeedingStatusResult feedingStatus,
            ...
            string discoveryMessage)
        {
            if (!careResult.success)
            {
                return CombineMessages(
                    CombineMessages(careResult.message, routineMessage),
                    discoveryMessage);
            }

            if (IsFeedingAction(careAction))
            {
                return FormatFeedingResultMessage(
                    careAction,
                    careResult,
                    feedingStatus,
                    routineMessage,
            ...
            if (careAction != MilkroomCareAction.Rest)
            {
                return CombineMessages(
                    CombineMessages(careResult.message, routineMessage),
                    discoveryMessage);
            ...
            {
                headline = "푹 쉬고 알에서 깨어났어요!";
            }
            else if (careResult.leveledUp)
            {
                headline = "푹 쉬고 레벨이 올랐어요!";
            }
            else if (feedingStatus.HasMessage)
            {
                headline = "푹 쉬어서 졸림과 불편한 몸 상태가 좋아졌어요!";
            }
            else
            {
                headline = "푹 쉬어서 졸림이 줄고 몸과 기분이 좋아졌어요!";
            }

            var friendlyRoutineMessage = string.Equals(
                    routineMessage,
                    GameManager.DailyRoutineRewardMessage,
                    System.StringComparison.Ordinal)
                ? $"오늘 돌봄도 끝! 코인 {GameManager.DailyRoutineMilkCoinReward}개, "
                    + $"우유방울 {GameManager.DailyRoutineMilkDropReward}개, "
                    + $"도감조각 {GameManager.DailyRoutineCollectionFragmentReward}개를 받았어요."
                : CollapseToOneLine(routineMessage);
            var detail = CombineInlineMessages(
                friendlyRoutineMessage,
                CollapseToOneLine(discoveryMessage));
            return CombineMessages(headline, detail);
        }

        private static string FormatFeedingResultMessage(
            MilkroomCareAction careAction,
            CareActionResult careResult,
            CheeseTama.Gameplay.Feeding.FeedingStatusResult feedingStatus,
            string routineMessage,
            string discoveryMessage)
        {
            var isSnack = careAction == MilkroomCareAction.FeedSnack;
            string headline;
            if (careResult.hatched)
            {
                headline = isSnack
            ...
            var detail = BuildFeedingResultDetail(
                feedingStatus.HasMessage,
                !string.IsNullOrWhiteSpace(routineMessage),
                !string.IsNullOrWhiteSpace(discoveryMessage));
            return CombineMessages(headline, detail);
        }

        private static string BuildFeedingResultDetail(
            bool hasStatusChange,
            bool hasRoutineReward,
            bool hasDiscovery)
        {
            if (hasStatusChange)
            {
                if (hasRoutineReward && hasDiscovery)
                {
                    return "상태 수치를 확인해요. 보상과 새 기록도 생겼어요.";
                }

                if (hasRoutineReward)
                {
                    return "상태 수치를 확인해요. 보상도 받았어요.";
                }

                if (hasDiscovery)
                {
                    return "상태 수치를 확인해요. 새 기록도 생겼어요.";
                }

                return "달라진 몸 상태는 상태 수치에서 확인해요.";
            }

            if (hasRoutineReward && hasDiscovery)
            {
                return "돌봄 보상을 받고 새 기록도 생겼어요.";
            }

            if (hasRoutineReward)
            {
                return "오늘 돌봄 보상을 받았어요.";
            }

            return hasDiscovery ? "새 기록도 생겼어요." : string.Empty;
        }

        private static bool IsFeedingAction(MilkroomCareAction careAction)
        {
            return careAction == MilkroomCareAction.FeedMilk
                || careAction == MilkroomCareAction.FeedWarmMilk
                || careAction == MilkroomCareAction.FeedColdMilk
                || careAction == MilkroomCareAction.FeedNuttyMilk
                || careAction == MilkroomCareAction.FeedRichMilk
                || careAction == MilkroomCareAction.FeedFermentedMilk
                || careAction == MilkroomCareAction.FeedCoffeeMilk
                || careAction == MilkroomCareAction.FeedStarMilk
                || careAction == MilkroomCareAction.FeedSnack;
        }

        private static string CombineInlineMessages(string primary, string secondary)
        {
            if (string.IsNullOrWhiteSpace(primary))
            {
                return secondary ?? string.Empty;
        ...
        private static string CollapseToOneLine(string value)
        {
