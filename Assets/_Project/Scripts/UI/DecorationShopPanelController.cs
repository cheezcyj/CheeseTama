    ...
    public sealed class DecorationShopPanelController : MonoBehaviour
    {
        ...
        private void Render()
        {
            ...
            for (var index = 0; index < items.Length; index += 1)
            {
                ...
                SetItemName(Get(itemNameTexts, index), item.displayName);
            ...
            SetInteractable(
                ...
                || DecorationShopRules.CanUnequip(selectedItem, snapshot));
            SetButtonLabel(
                purchaseButton,
                snapshot.Owns(selectedItem.id) ? "보유 중" : "구매");
            SetButtonLabel(
                equipButton,
                GetEquipButtonLabel(selectedItem, snapshot));
        }
        ...
        private static string GetEquipButtonLabel(
            DecorationDefinition item,
            DecorationShopSnapshot currentSnapshot)
        {
            if (currentSnapshot.GetEquippedId(item.slot) != item.id)
            ...
            return item.defaultOwned ? "기본 장식" : "장착 해제";
        }
        ...
        private static void SetItemName(Text label, string value)
        {
            SetText(label, value);
            if (label != null)
            {
