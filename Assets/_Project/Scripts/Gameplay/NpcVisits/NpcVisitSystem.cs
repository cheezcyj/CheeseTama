...
{
    public sealed class NpcVisitContext
    {
        public NpcVisitContext(bool stainGuestUnlocked, string seasonOneFinalChoiceId)
        {
            StainGuestUnlocked = stainGuestUnlocked;
            SeasonOneFinalChoiceId = (seasonOneFinalChoiceId ?? string.Empty).Trim();
        }

        public bool StainGuestUnlocked { get; }
        public string SeasonOneFinalChoiceId { get; }
        public static NpcVisitContext Default { get; } = new NpcVisitContext(false, string.Empty);
    }
    ...
    public sealed class NpcVisitOffer
    {
        ...
        public string Message => !string.IsNullOrEmpty(MessageOverride)
            ? MessageOverride
            : Visitor == null || Visitor.Introductions.Count == 0
                ? string.Empty
    ...
    public sealed class NpcVisitResolutionResult
    {
        public NpcVisitResolutionResult(
            ...
            bool weeklyRewardApplied = false,
            bool keepsakeUnlocked = false)
        {
            Applied = applied;
            OccurrenceId = occurrenceId ?? string.Empty;
            ...
            MilkCoins = Math.Max(0, milkCoins);
            CollectionFragments = Math.Max(0, collectionFragments);
            WeeklyRewardApplied = weeklyRewardApplied;
            KeepsakeUnlocked = keepsakeUnlocked;
        }
        ...
        public int MilkCoins { get; }
        public int CollectionFragments { get; }
        public bool WeeklyRewardApplied { get; }
        public bool KeepsakeUnlocked { get; }
    }
    ...
    public sealed class NpcVisitSystem
    {
        ...
        public const string StainGuestId = "stain_guest";
        public const string StainGuestKeepsakeRecordId = "npc_keepsake_stain_guest_blue_thread";
        public const string SeasonOneFinalEpisodeId = "drawer_chapter_07_shared_dawn";
        public const string SeasonOneOpenChoiceId = "open_before_sunrise";
        public const string SeasonOneCloseChoiceId = "close_after_reading";
        public const int StainGuestWeeklyCollectionFragmentReward = 1;
        ...
        private static readonly NpcVisitDefinition[] Definitions =
        {
            ...
            new NpcVisitDefinition(
                ...
                new[]
                {
                    "얼룩 손님이 문턱을 넘어 푸른 실 한 가닥을 조용히 내밀었어요.",
                    "얼룩 손님이 창가의 빈 기록칸을 살피며 다시 찾아왔어요.",
                    "익숙한 푸른 매듭이 먼저 흔들리고 얼룩 손님이 자리에 앉았어요."
                },
                new[]
                {
                    new NpcVisitChoiceDefinition(
                        "share_blue_thread",
                        "푸른 실 함께 정리하기",
                        "흩어진 기억 실을 나란히 묶어 이번 주의 방문 기록을 남겼어요.",
                        new StatEffect { mood = 4, affection = 2 },
                        affinity: 2),
                    new NpcVisitChoiceDefinition(
                        "inspect_window_clue",
                        "창가 단서 함께 살피기",
                        "창틀의 오래된 흔적에서 다음 꿈 기록으로 이어지는 푸른 선을 찾았어요.",
                        new StatEffect { maturation = 3, mood = 2 },
                        affinity: 1)
                })
        };

        private static readonly IReadOnlyDictionary<string, NpcVisitSelectionRule>
            SelectionRules = new Dictionary<string, NpcVisitSelectionRule>(StringComparer.Ordinal)
            {
                {
                    MilkyDoctorId,
                    new NpcVisitSelectionRule(
                        (_, __) => true,
                        (tama, history) =>
                            ((tama?.stats?.health ?? 100) < 75 ? 4 : 0)
                            + ((tama?.stats?.hunger ?? 100) < 45 ? 3 : 0)
                            + ((tama?.stats?.overfullness ?? 0) > 0 ? 3 : 0))
                },
                {
                    FermentationFairyId,
                    new NpcVisitSelectionRule(
                        (_, __) => true,
                        (tama, history) =>
                            ((tama?.stats?.maturation ?? 0) >= 55 ? 4 : 0)
                            + ((history?.waitHours ?? 0) > 0 ? 2 : 0)
                            + ((history?.milkFeeds ?? 0) >= 3 ? 1 : 0))
                },
                {
                    MilkCatId,
                    new NpcVisitSelectionRule(
                        (_, __) => true,
                        (tama, history) =>
                            ((tama?.stats?.mood ?? 100) < 65 ? 4 : 0)
                            + ((history?.playSessions ?? 0) > 0 ? 2 : 0)
                            + ((history?.petSessions ?? 0) > 0 ? 1 : 0))
                },
                {
                    StainGuestId,
                    new NpcVisitSelectionRule(
                        (context, _) => context?.StainGuestUnlocked == true,
                        (_, history) => 3 + Math.Min(5, (history?.totalCareActions ?? 0) / 10))
                }
            };
        ...
        {
            return TryGetPending(state, NpcVisitContext.Default, out offer);
        }

        public bool TryGetPending(
            NpcVisitSaveData state,
            NpcVisitContext context,
            out NpcVisitOffer offer)
        {
            ...
            offer = new NpcVisitOffer(
                ...
                false,
                ResolveIntroductionOverride(visitor, context));
            return true;
        }
            ...
            out NpcVisitOffer offer)
        {
            ...
            state.EnsureRuntimeDefaults();
            if (TryGetPending(state, context, out offer))
            {
                return true;
            }
            ...
            var visitor = SelectVisitor(state, tama, careHistory, now, visitorRoll, context);
            if (visitor == null)
            {
                return false;
            }
            ...
            if (string.Equals(visitor.Id, StainGuestId, StringComparison.Ordinal)
                && context != null
                && !string.IsNullOrEmpty(context.SeasonOneFinalChoiceId))
            {
                state.stainGuestEchoChoiceId = context.SeasonOneFinalChoiceId;
            }
            ...
            out NpcVisitResolutionResult result)
        {
            ...
            var weeklyRewardApplied = false;
            var keepsakeUnlocked = false;
            var totalCollectionFragments = choice.CollectionFragments;
            if (string.Equals(visitor.Id, StainGuestId, StringComparison.Ordinal))
            {
                ApplyStainGuestWeeklyVisit(
                    state,
                    economy,
                    now,
                    out weeklyRewardApplied,
                    out keepsakeUnlocked);
                if (weeklyRewardApplied)
                {
                    totalCollectionFragments = SaturatingAdd(
                        totalCollectionFragments,
                        StainGuestWeeklyCollectionFragmentReward);
                }
            }
            ...
            state.pending.Clear();
            var resultMessage = choice.ResultMessage;
            if (weeklyRewardApplied)
            {
                resultMessage += " 푸른 실 기념품과 주간 도감 조각을 받았어요.";
            }
        ...
        private static NpcVisitDefinition SelectVisitor(
            ...
            NpcVisitContext context)
        {
            var safeContext = context ?? NpcVisitContext.Default;
            ...
            for (var index = 0; index < Definitions.Length; index += 1)
            {
                if (string.Equals(
                    ...
                    && HasStainGuestWeeklyReceipt(state, now))
                {
                    continue;
                }

                if (!SelectionRules.TryGetValue(Definitions[index].Id, out var rule)
                    || !rule.IsAvailable(safeContext, state))
                {
                    continue;
                }

                var score = rule.Score(tama, careHistory);
                if (score > best)
                {
                    best = score;
                    candidates.Clear();
        ...
        private static string ResolveIntroductionOverride(
            NpcVisitDefinition visitor,
            NpcVisitContext context)
        {
            if (visitor == null
                || !string.Equals(visitor.Id, StainGuestId, StringComparison.Ordinal)
                || context == null)
            {
                return string.Empty;
            }
            ...
            if (string.Equals(
                    context.SeasonOneFinalChoiceId,
                    SeasonOneOpenChoiceId,
                    StringComparison.Ordinal))
            {
                return "활짝 열었던 문을 기억한 얼룩 손님이 푸른 실 기념품을 들고 들어왔어요.";
            }

            if (string.Equals(
                    context.SeasonOneFinalChoiceId,
                    SeasonOneCloseChoiceId,
                    StringComparison.Ordinal))
            {
                return "함께 닫아 둔 기록의 책갈피가 빛나자 얼룩 손님이 조용히 문을 두드렸어요.";
            }

            return string.Empty;
        }

        private static void ApplyStainGuestWeeklyVisit(
            NpcVisitSaveData state,
            EconomySaveData economy,
            DateTimeOffset now,
            out bool rewardApplied,
            out bool keepsakeUnlocked)
        {
            rewardApplied = false;
            ...
            state.stainGuestWeeklyReceiptKeys ??= new List<string>();
            var receiptKey = BuildStainGuestWeeklyReceiptKey(now);
            if (state.stainGuestWeeklyReceiptKeys.Contains(receiptKey))
            {
                return;
            }

            state.stainGuestWeeklyReceiptKeys.Add(receiptKey);
            while (state.stainGuestWeeklyReceiptKeys.Count
                > NpcVisitSaveData.MaximumStainGuestWeeklyReceipts)
            {
                state.stainGuestWeeklyReceiptKeys.RemoveAt(0);
            }
            ...
            economy.collectionFragments = SaturatingAdd(
                ...
                StainGuestWeeklyCollectionFragmentReward);
            rewardApplied = true;
            if (!state.stainGuestKeepsakeUnlocked)
            {
                state.stainGuestKeepsakeUnlocked = true;
                keepsakeUnlocked = true;
            }
        }

        private static bool HasStainGuestWeeklyReceipt(
            NpcVisitSaveData state,
            DateTimeOffset now)
        {
            return state?.stainGuestWeeklyReceiptKeys != null
                && state.stainGuestWeeklyReceiptKeys.Contains(
                    BuildStainGuestWeeklyReceiptKey(now));
        }

        private static string BuildStainGuestWeeklyReceiptKey(DateTimeOffset now)
        {
            var weekStart = now.Date.AddDays(-(((int)now.DayOfWeek + 6) % 7));
            return "stain-guest-week:"
        ...
        private sealed class NpcVisitSelectionRule
        {
            public NpcVisitSelectionRule(
                Func<NpcVisitContext, NpcVisitSaveData, bool> isAvailable,
                Func<CheeseTamaModel, CareHistorySaveData, int> score)
            {
                IsAvailable = isAvailable;
                Score = score;
            }

            public Func<NpcVisitContext, NpcVisitSaveData, bool> IsAvailable { get; }
            public Func<CheeseTamaModel, CareHistorySaveData, int> Score { get; }
        }
    }
}
