...
namespace CheeseTama.Gameplay.NpcVisits
{
    ...
    public sealed class NpcRelationshipEpisodeSystem
    {
            ...
            int affinityBonus)
        {
            ...
            var effectiveAffinityGain = choice.Affinity <= 0
                ? choice.Affinity
                : SaturatingAdd(choice.Affinity, Math.Max(0, affinityBonus));
            var affinityAfter = Math.Min(
                99,
                SaturatingAdd(affinityBefore, effectiveAffinityGain));
