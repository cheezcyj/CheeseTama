            ...
            IList<string> claimedKeys)
        {
            ...
            for (var level = FirstRewardLevel; level <= normalizedLevel; level += 1)
            {
                ...
                switch (level)
                {
                    case 2:
                        milkCoins += 4;
                        milestoneMessages.Add("레벨 2 반응 이야기가 열렸어요");
                        break;
                    case 3:
                        milkDrops += 2;
                        milestoneMessages.Add($"레벨 3 요리 도움말: {GetRecipeHint(milkId)}");
                        break;
                    case 4:
                        milkCoins += 8;
                        fragments += 1;
                        milestoneMessages.Add("레벨 4 전용 성장 기록을 발견했어요");
                        break;
                    case 5:
                        milkCoins += 12;
                        milkDrops += 3;
                        fragments += 1;
                        milestoneMessages.Add("레벨 5 완전 성장 기록을 달성했어요");
                        break;
                }
            }
