...
namespace CheeseTama.Gameplay.Care
{
    public sealed class CareActionSystem
    {
        ...
        public CareActionResult FeedSnack(CheeseTamaModel tama)
        {
            ...
            LastFeedingStatusResult = feedingStatus;
            ApplyStomachRiskHealthPenalty(tama, hungerBefore, 10);
            tama.stats.ClampAll();
            ...
            DateTimeOffset? localTime)
        {
            ...
            LastFeedingStatusResult = feedingStatus;
            ApplyStomachRiskHealthPenalty(tama, hungerBefore, snack.hunger);
            tama.stats.ClampAll();
        ...
        public CareActionResult Play(CheeseTamaModel tama)
        {
            ...
            var feedingStatus = feedingStatusSystem.RecoverByPlay(tama);
            LastFeedingStatusResult = feedingStatus;
            tama.stats.ClampAll();
        ...
        private CareActionResult AddCareProgress(CheeseTamaModel tama, int progress, string message)
        {
            ...
            var baseProgress = Mathf.Max(0, progress);
            var appliedProgress = feedingStatusSystem.CalculateCareProgress(tama, baseProgress);
            message = CombineMessages(
                message,
                feedingStatusSystem.BuildCareEfficiencyMessage(tama, baseProgress, appliedProgress));
            message = CombineMessages(
                message,
                feedingStatusSystem.BuildTemporaryPreferenceMessage(tama));
        ...
        private static void ApplyStomachRiskHealthPenalty(
            CheeseTamaModel tama,
            int hungerBefore,
            int hungerGain)
        {
            if (tama?.stats != null
                && FeedingStatusSystem.IsOvereating(hungerBefore, hungerGain))
            {
                tama.stats.health -= FeedingStatusSystem.StomachRiskHealthPenaltyPerOvereating;
            }
        }
