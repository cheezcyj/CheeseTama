...
namespace CheeseTama.Gameplay.Stats
{
    public readonly struct TimeProgressionResult
    {
        ...
        public TimeProgressionResult(
            ...
            bool heavinessRecovered = false,
            bool sugarOverloadRecovered = false,
            bool fantasyEchoRecovered = false,
            bool lethargyRecovered = false,
            bool stomachRiskRecovered = false,
            bool traitDistortionRecovered = false)
        {
        ...
        public string ToSummary(string prefix)
        {
            ...
            if (heavinessRecovered)
            {
                aftereffectText += ", 무거움 회복";
            }

            if (sugarOverloadRecovered)
            {
                aftereffectText += ", 단것 과다 회복";
            }

            if (fantasyEchoRecovered)
            {
                aftereffectText += ", 환상 잔향 회복";
            }

            if (lethargyRecovered)
            {
                aftereffectText += ", 나른함 회복";
            }

            if (stomachRiskRecovered)
            {
                aftereffectText += ", 배탈 위험 회복";
            }

            if (traitDistortionRecovered)
            {
                aftereffectText += ", 성격 흔들림 회복";
            }

            return $"{prefix} {hours}시간이 지났어요. 배부름 {hungerDelta}, 기분 {moodDelta}, 깨끗함 {cleanlinessDelta}, 졸림 +{sleepinessDelta}{healthText}{overfullnessText}{aftereffectText}.";
        }
    }
    ...
    public sealed class TimeProgressionSystem
    {
        ...
        public TimeProgressionResult ApplyCareTicks(CheeseTamaModel tama, int careTicks)
        {
            ...
            return new TimeProgressionResult(
                ...
                feedingStatus.heavinessRecovered,
                feedingStatus.sugarOverloadRecovered,
                feedingStatus.fantasyEchoRecovered,
                feedingStatus.lethargyRecovered,
                feedingStatus.stomachRiskRecovered,
                feedingStatus.traitDistortionRecovered);
        }
    }
}
