...
namespace CheeseTama.Gameplay.Growth
{
    ...
    public sealed class StarEggEmmentalEvolutionSystem
    {
            ...
            StarLegacySaveData state)
        {
            ...
            if (IsStarEggOrigin(tama) && !CanContinueStarLineage(tama, state))
            {
        ...
        public static bool CanContinueStarLineage(CheeseTamaModel tama, StarLegacySaveData state)
        {
            return tama != null
                && state?.maturationCycle != null
                && IsStarEggOrigin(tama)
                && string.Equals(
                    tama.evolutionId,
                    EmmentalEvolutionId,
                    StringComparison.Ordinal)
                && state.maturationCycle.completedCycles > 0;
        }
            ...
            StarEggGenerationEligibilityStatus status)
        {
            switch (status)
            {
                ...
                case StarEggGenerationEligibilityStatus.PendingLineageTraitSelection:
                    return StarEggGenerationStartStatus.PendingLineageTraitSelection;
