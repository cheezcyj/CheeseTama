...
namespace CheeseTama.Gameplay.Decorations
{
    ...
    public sealed class DecorationShopSnapshot
    {
        ...
        public static DecorationShopSnapshot CreateDefault(
            int milkCoins = 0,
            int milkDrops = 0,
        ...
        internal DecorationShopSnapshot WithPurchase(DecorationDefinition item)
        {
            ...
            return new DecorationShopSnapshot(
                ...
                collectionFragments - item.collectionFragmentCost,
                nextOwnedIds,
                equippedWallId,
                equippedFloorId,
                equippedAccentId,
        ...
        internal DecorationShopSnapshot WithUnequipped(DecorationDefinition item)
        {
            var defaultItemId = DecorationCatalog.GetDefault(item.slot)?.id ?? string.Empty;
            return new DecorationShopSnapshot(
                milkCoins,
                milkDrops,
                collectionFragments,
                ownedItemIds,
                item.slot == DecorationSlot.Wall ? defaultItemId : equippedWallId,
                item.slot == DecorationSlot.Floor ? defaultItemId : equippedFloorId,
                item.slot == DecorationSlot.Accent ? defaultItemId : equippedAccentId,
                item.slot == DecorationSlot.Window ? defaultItemId : equippedWindowId,
                item.slot == DecorationSlot.Shelf ? defaultItemId : equippedShelfId,
                item.slot == DecorationSlot.Bedside ? defaultItemId : equippedBedsideId);
        }
    ...
    public static class DecorationShopRules
    {
        public static bool CanPurchase(DecorationDefinition item, DecorationShopSnapshot snapshot)
        {
            return item != null
                ...
                && snapshot.collectionFragments >= item.collectionFragmentCost;
        }
        ...
        public static bool CanUnequip(DecorationDefinition item, DecorationShopSnapshot snapshot)
        {
            var defaultItem = item == null ? null : DecorationCatalog.GetDefault(item.slot);
            return item != null
                && snapshot != null
                && snapshot.Owns(item.id)
                && snapshot.GetEquippedId(item.slot) == item.id
                && (defaultItem == null || defaultItem.id != item.id);
        }
        ...
        public static DecorationTransactionResult Unequip(
            string itemId,
            DecorationShopSnapshot snapshot)
        {
            snapshot ??= DecorationShopSnapshot.CreateDefault();
            ...
            if (snapshot.GetEquippedId(item.slot) != item.id)
            {
                return Result(
                    DecorationTransactionStatus.NotEquipped,
                    item,
                    snapshot,
                    $"{item.displayName}은(는) 현재 장착되어 있지 않아요.");
            }

            if (!CanUnequip(item, snapshot))
            {
                return Result(
                    DecorationTransactionStatus.DefaultDecorationRequired,
                    item,
                    snapshot,
                    "기본 장식은 해제할 수 없어요.");
            }

            var fallbackItem = DecorationCatalog.GetDefault(item.slot);
            var message = fallbackItem == null
                ? $"{item.displayName} 장착을 해제했어요."
                : $"{item.displayName} 장착을 해제하고 {fallbackItem.displayName}로 바꿨어요.";
            return Result(
                DecorationTransactionStatus.Success,
                item,
                snapshot.WithUnequipped(item),
                message);
        }

        public static DecorationTransactionResult ToggleEquip(
            string itemId,
            DecorationShopSnapshot snapshot)
        {
            snapshot ??= DecorationShopSnapshot.CreateDefault();
            var item = DecorationCatalog.Find(itemId);
            return item != null && snapshot.GetEquippedId(item.slot) == item.id
                ? Unequip(itemId, snapshot)
                : Equip(itemId, snapshot);
        }
        ...
        public static string FormatPrice(DecorationDefinition item)
        {
            ...
            {
                parts.Add($"코인 {item.milkCoinCost}");
            }

            if (item.milkDropCost > 0)
            {
                parts.Add($"우유방울 {item.milkDropCost}");
            }

            if (item.collectionFragmentCost > 0)
            {
                parts.Add($"도감 조각 {item.collectionFragmentCost}");
            }
