...
namespace CheeseTama.Gameplay.Decorations
{
    public sealed class DecorationRoomPresenter : MonoBehaviour
    {
        ...
        private System.Func<DecorationPlacementSnapshot> placementSnapshotProvider;
        ...
        public void Refresh()
        {
            ...
            ApplyFloorDecoration(snapshot.equippedFloorId);
            ...
            RebuildBedsideSlot(snapshot.equippedBedsideId);
            ApplyWorkshopSnapshot(workshop);
            var placementSnapshot = placementSnapshotProvider?.Invoke()
            ...
            ApplyPlacementSnapshot(DecorationPlacementSystem.ReconcileWithEquippedSlots(
                placementSnapshot,
                snapshot.equippedAccentId,
                snapshot.equippedShelfId,
                snapshot.equippedBedsideId));
        ...
        public void ConfigurePlacementProvider(
            System.Func<DecorationPlacementSnapshot> getPlacementSnapshot)
        {
            placementSnapshotProvider = getPlacementSnapshot;
            Refresh();
        }

        public void ApplyPlacementSnapshot(DecorationPlacementSnapshot snapshot)
        {
            snapshot ??= new DecorationPlacementSnapshot(null);
            ApplyPlacementPose(
                snapshot.Find(DecorationSlot.Accent),
                accentVisual,
                appliedAccentId,
                1.15f,
                0.34f,
                rotateAroundVerticalAxis: true);
            ApplyPlacementPose(
                ...
                0.62f,
                0.18f,
                rotateAroundVerticalAxis: false);
            ApplyPlacementPose(
                snapshot.Find(DecorationSlot.Bedside),
                bedsideVisual,
                appliedBedsideId,
                0.86f,
                0.30f,
        ...
        public Transform GetMovableVisual(DecorationSlot slot)
        {
            return slot switch
            {
                DecorationSlot.Accent => accentVisual != null ? accentVisual.transform : null,
                DecorationSlot.Shelf => shelfVisual != null ? shelfVisual.transform : null,
                DecorationSlot.Bedside => bedsideVisual != null ? bedsideVisual.transform : null,
                _ => null
            };
        }
        ...
        private void ApplyFloorDecoration(string itemId)
        {
            ...
            ApplyColor(floorRenderer, GetFloorColor(itemId));
        }
        ...
        private void ApplyWorkshopSnapshot(DecorationWorkshopRenderSnapshot snapshot)
        {
            ...
            ClearColorOverrides(transform.Find("Window_Model")?.gameObject);
            ApplyWorkshopTint(
                windowVisual,
            ...
            Color defaultColor)
        {
            ...
            if (visual != null && appliedId == itemId)
            {
                return;
            }
        ...
        private void RebuildBedsideSlot(string itemId)
        {
            RebuildSimpleSlot(
                bedsideAnchor,
                ref bedsideVisual,
                ref appliedBedsideId,
                itemId,
                DecorationCatalog.StarPlushId,
                PrimitiveType.Sphere,
                new Vector3(0.34f, 0.28f, 0.22f),
                new Color(0.95f, 0.72f, 0.2f),
                new Color(0.92f, 0.92f, 0.84f));
        }

        private static void ApplyPlacementPose(
            DecorationPlacementEntry placement,
            GameObject visual,
            string equippedDecorationId,
            float maximumLocalX,
            float maximumLocalY,
            bool rotateAroundVerticalAxis)
        {
            if (visual == null)
            {
                return;
            ...
            var matchesEquipped = placement != null
                && string.Equals(
                    placement.DecorationId,
                    equippedDecorationId,
                    System.StringComparison.Ordinal);
            var normalizedX = matchesEquipped ? placement.NormalizedX : 0f;
            var normalizedY = matchesEquipped ? placement.NormalizedY : 0f;
            var rotationDegrees = matchesEquipped ? placement.RotationDegrees : 0f;
            visual.transform.localPosition = new Vector3(
                normalizedX * maximumLocalX,
                normalizedY * maximumLocalY,
                0f);
            visual.transform.localRotation = rotateAroundVerticalAxis
        ...
        private static Color GetFloorColor(string id)
        {
            return id switch
            {
                DecorationCatalog.CreamRugId => new Color(0.94f, 0.82f, 0.58f, 1f),
